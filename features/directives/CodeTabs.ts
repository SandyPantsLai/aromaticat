...
function replaceCodeTabs(node: MdxJsxFlowElement, ancestors: Array<Parent>) {
  if (node.children.some((child) => child.type !== 'code')) {
    throw new Error(
      `CodeTabs must contain only fenced code blocks (each may use name= in the fence meta for tab labels). Found:\n\n${JSON.stringify(node.children, null, 2)}`
    )
  }

  const parent = ancestors[ancestors.length - 1]
  const remappedChildren = node.children.map(
    (code: Code, idx): MdxJsxFlowElement => {
...
function getFileName(node: Code): string | undefined {
  return node.meta
    ?.split(/\s+/)
