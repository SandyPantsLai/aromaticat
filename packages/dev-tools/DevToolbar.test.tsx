...
describe('DevToolbar', () => {
  ...
  describe('when not in local development', () => {
    ...
    it(
      'returns null and does not render anything',
      async () => {
        vi.resetModules()
  ...
  describe('Dev tools sheet', () => {
    beforeEach(() => {
      vi.stubEnv('NODE_ENV', 'development')
      localStorage.setItem('dev-telemetry-toolbar-enabled', 'true')
    })

    it('opens sheet with ConfigCat flags visible', async () => {
      vi.resetModules()
      ...
      expect(screen.getByText('example_flag')).toBeInTheDocument()
    })
  })
})
