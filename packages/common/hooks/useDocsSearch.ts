...
import { getSupabasePublicApiKey } from '../supabasePublicClientKey'

const NUMBER_SOURCES = 1

const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
const SUPABASE_PUBLIC_KEY = getSupabasePublicApiKey()
...
function reshapeResults(result: unknown): Page | null {
  ...
  const rawType = result.type as string
  const normalizedType: PageType =
    rawType === 'troubleshooting' || rawType === 'blog' ? PageType.Blog : (rawType as PageType)
