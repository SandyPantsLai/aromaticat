...
export const useActiveMenuLabel = (menu: typeof GLOBAL_MENU_ITEMS) => {
  ...
  useEffect(() => {
    ...
    const matchesHref = (href: string | undefined) =>
    ...
    for (let index = 0; index < menu.length; index++) {
      ...
      if (matchesHref(section[0].href)) {
        return setActiveLabel(section[0].label)
      }

      if (section[0].level === 'fragrance-notes' && pathname.startsWith('/fragrance-notes')) {
        return setActiveLabel(section[0].label)
      }
                ...
                >
                  ...
                  >
                    {section[0].label === 'Home' ? (
                      <MenuIconPicker icon={section[0].icon ?? 'home'} />
                    ) : (
                      section[0].label
                    )}
                  </NavigationMenuTrigger>
